...
gof = [
    ...
    "Many distinct and unrelated operations need to be performed on node objects in a heterogeneous aggregate structure. You want to avoid ‘polluting’ the node classes with these operations. And, you do not want to have to query the type of each node and cast the pointer to the appropriate type before performing the desired operation"]
douglass = ["One of the key problems with dynamic memory allocation is memory fragmentation. Memory fragmentation is the random intermixing of free and allocated memory in the heap. For memory fragmentation to occur, the two conditions must be met (1) the order of memory allocation is unrelated to the order in which it is released, and (2) Memory is allocated in various sizes from the heap.",
