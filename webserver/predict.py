...
def main(design_problem: str = ""):
    ...
    df = file_to_df(os.path.join(os.path.dirname(__file__), f"GOF Patterns (2.0).csv"))
    ...
    for a, algo in enumerate(algorithms):
        ...
        predicted_cluster = df[algo].iloc[-1]

        df_problem_cluster = (
            df.loc[df[algo] == predicted_cluster][["name", "category", algo]]
            .iloc[:-1]
            .copy()
        )
        ...
        print(f"Category is most likely {predicted_category}\n")
        output.append(f"Category is most likely {predicted_category}")
